...
derivative tc27D
{
    ...
    memory pfls0
    {
        ...
        size = 1M;
        type = rom;
        map not_cached (dest=bus:sri, dest_offset=0x80000000,           size=1M);
...
/*-------OTA program flash0-------*/
    memory ota_pfls0
    {
        ...
        size = 512k;
        type = rom;
        map not_cached (dest=bus:sri, dest_offset=0x80100000, size=512k);
        map not_cached (dest=bus:sri, dest_offset=0xa0100000, reserved, size=512k);
    }
    section_layout :vtc:linear
    {
        group myPFlsGroup0 (ordered, run_addr=mem:ota_pfls0)
        {
            select ".myfunc";
            select ".otaSection0";
        }
...
/*-------OTA program flash1-------*/
    memory ota_pfls1
    {
        ...
        size = 512k;
        type = rom;
        map not_cached (dest=bus:sri, dest_offset=0x80180000, size=512k);
        map not_cached (dest=bus:sri, dest_offset=0xa0180000, reserved, size=512k);
    }
    section_layout :vtc:linear
    {
        group myPFlsGroup1 (ordered, run_addr=mem:ota_pfls1)
        {
            select ".otaSection1";
        }
    }
    ...
    memory dfls0
    {
        ...
        size = 1m+16k;
        type = reserved nvram;
        map (dest=bus:sri, dest_offset=0xaf000000, size=384k  );    // origin : 384k
    }

/*-------OTA data section-------*/
    section_layout :vtc:linear
    {
        group myDFlsGroup (ordered, run_addr=0xaf000000)
        {
            select ".otaDataSection";
        }
    }
